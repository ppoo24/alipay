...
AlipaySubmit.prototype.buildRequestHttpInFile = function (para_temp, file_para_name, file_name, callback) {
    //待请求参数数组
    var para = this.buildRequestPara(para_temp);
    if (file_para_name) { throw new Error('上传文件功能暂不被支持'); } //ws=
    para[file_para_name] = "@" + file_name; //ws=【注：这里应该无法上传文件的，这里仅仅是照搬了php-curl的代码，getHttpResponsePOST中的代码目前无法支持文件上传】
    //远程获取数据
...
AlipaySubmit.prototype.query_timestamp = function (callback) {
    var url = this.alipay_gateway_new + "service=query_timestamp&partner=" + this.alipay_config['partner'].toLowerCase().trim() + '&_input_charset=' + this.alipay_config['input_charset'].toLowerCase().trim();
