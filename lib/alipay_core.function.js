...
module.exports = {
    /**
     * 把键值对所有元素，按照“参数=参数值”的模式用“&”字符拼接成字符串
     * @param para 需要拼接的键值对
     * return 拼接完成以后的字符串
     */
    ...
    /**
     * 把键值对所有元素，按照“参数=参数值”的模式用“&”字符拼接成字符串，并对字符串做urlencode编码
     * @param para 需要拼接的键值对
     * return 拼接完成以后的字符串
     */
    createLinkstringUrlencode: function (para) {
        return qs.stringify(para);
    },
    /**
     * 除去键值对中的空值和签名参数
     * @param para 签名参数键值对
     * return 去掉空值与签名参数后的新签名参数键值对
     */
    paraFilter: function (para) {
        var para_filter = {};
        for (var key in para){
     ...
     * 远程获取数据，POST模式
     * 注意：
     * # 文件夹中cacert.pem是SSL证书请保证其路径有效，目前默认路径是：本模块下'./cacert.pem'
     ...
     * return 远程输出的数据
     */
    getHttpResponsePOST: function (url, cacert_url, para, input_charset, callback) {
     ...
     * return 远程输出的数据
     */
    getHttpResponseGET: function (url, cacert_url, callback) {
        var parsed_url = require('url').parse(url);
        var options = {
            hostname: parsed_url.host,
            port: 443,
     ...
     * return 编码后的字符串
     */
    charsetEncode: function (input, _output_charset, _input_charset) {
        throw new Error('此函数暂未实现');
        // var output = "";
     ...
     * return 解码后的字符串
     */
    charsetDecode: function(input, _input_charset, _output_charset){
        throw new Error('此函数暂未实现');
        // var output = "";
